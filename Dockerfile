...
RUN apt-get update && apt-get install -y --no-install-recommends \
    wget squashfs-tools libgl1-mesa-dri libgles2 \
    ...
    && OFFSET=$(python3 -c "d=open('/tmp/UltraStarDeluxe.AppImage','rb').read(); print(d.index(b'hsqs'))") \
    && unsquashfs -offset "$OFFSET" -d /opt/usdx /tmp/UltraStarDeluxe.AppImage \
